...
class config():
    def __init__(self):
        ...
        self.input_size = 9  # Number of input features
        ...
        self.dataset_path = './results/dataset_whole.npy'
        self.date_path = './results/date.npy'
        # Other parameters
...
# Load precomputed prediction water level results for Lake Superior and Lake Michigan-Huron
wl_sup = np.load('./dataset/superior_prediction_results.npy')
wl_mh = np.load('./dataset/michiganhuron_prediction_results.npy')

# Dataset feature mapping:
# water_level_Erie (index 0), water_level_Superior (index 1), water_level_MichiganHuron (index 2), 
# temp_lake (index 3), temp_land (index 4), 
# wspd_lake (index 5), wspd_land (index 6), lst (index 7), precipitation (index 8)
...
# Calculate the moving average and replace the observed Lake Superior and Michigan-Huron water levels with predicted values in training dataset
dataset = cal_moving_ave(dataset, wl_p.moving_length)
dataset[-len(wl_sup[:-wl_p.test_length]):, 1] = wl_sup[:-wl_p.test_length]
dataset[-len(wl_mh[:-wl_p.test_length]):, 2] = wl_mh[:-wl_p.test_length]
# Normalize dataset
...
date, dataset_original, dataset_new = generate_prediction_data(date_test, date_original, dataset_test, dataset_original, wl_p, wl_sup, wl_mh)
