...
def generate_prediction_data(date_test, date_original, dataset_test, dataset_original, parameter, wl_sup):
    ...
    # Calculate the moving average and replace the observed Lake Superior water levels with predicted values in whole dataset
    dataset = cal_moving_ave(dataset, parameter.moving_length)
    dataset[-len(wl_sup):, 1] = wl_sup
...
def final_test(dataset, parameter, model_1, model_2, model_3, criterion, parameter_list):
    ...
    for c in range(num):    
        ...
        pred_1, _ = model_1.forward(input_x_1)
        pred_1 = pred_1.squeeze(1)
        pred_2, _ = model_2.forward(input_x_2)
...
def test_analysis(date, dataset, dataset_original, max_dic, min_dic, results, targets, parameter, scaler):
    ...
    np.save('./results/average_total.npy', np.array(plot_average))
    ...
    # Define axis limits
    ax1.set_ylim(175, 178)
    ax2.set_ylim(175*3.281, 178*3.281)
    ...
    plt.show()
    np.save('./results/michiganhuron_water_level_test.npy', plot_preds)
    np.save('../Lake_Erie_trained/dataset/michiganhuron_water_level_test.npy', plot_preds)
    return error, error_pearson, plot_targets, plot_preds