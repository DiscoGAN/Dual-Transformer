...
def final_test(dataset, parameter, model_1, model_2, model_3, criterion):
    ...
    for c in range(num):    
        ...
        input_x_2 = fin_dataset[c*parameter.batch_size:(c+1)*parameter.batch_size, parameter.cycle_num_before:parameter.cycle_num_before+parameter.cycle_num_later, 1:parameter.input_size] # Excluding Michigan-Huron lake level
...
def test_analysis(date, dataset, dataset_original, max_dic, min_dic, results, targets, parameter, scaler):
    ...
    for num in range(parameter.predict_long):
        ...
        for mon in range(12):
            ...
            ax1.text(p_x[0], y2[0] - 0.08, min_dic_new[mon][0], size=11, weight='normal')
    ...
    ax1.set_ylim(175, 178.3)
    ax2.set_ylim(175*3.281, 178.3*3.281)
