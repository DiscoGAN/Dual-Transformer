...
# Define the Transformer configuration class
class Transconfig():
    def __init__(self):
        self.d_model = 7          # Dimension of input features -- default
        ...
        self.fc_p = 128           # Fully connected layer output size
        self.output_size = 1      # Output dimension
        self.batch_size = 256     # Batch size
        self.cycle_num = 5        # Number of time steps in input sequence -- default
...
class ScaledDotProductAttention(torch.nn.Module):
    ...
    def forward(self, Q, K, V):  # Q, K, V shapes: [batch_size, n_heads, seq_len, d_k]
        scores = torch.matmul(Q, K.transpose(-1, -2)) / np.sqrt(transconfig.d_k)  # Compute scaled dot-product attention scores
        attn = torch.nn.Softmax(dim=-1)(scores)  # Apply softmax along last dimension
...
class PoswiseFeedForwardNet(torch.nn.Module):
    ...
    def forward(self, inputs):  # inputs: [batch_size, seq_len, d_model]
        ...
        return torch.nn.LayerNorm(transconfig.d_model)(output + residual)  # Apply residual connection and layer normalization
...
# Modified Transformer model with Encoder and Prediction Layer
class Transformer_half_modify(torch.nn.Module):
    def __init__(self, parameter_list, parameter):
        ...
        transconfig.cycle_num = parameter_list[1]
        transconfig.d_model = parameter.input_size - 1
